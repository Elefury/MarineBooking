<div class="bg-white rounded-xl shadow-lg overflow-hidden max-w-2xl mx-auto mt-4">
    <div class="bg-gradient-to-r from-blue-600 to-green-500 p-6 text-white">
        <div class="flex items-center space-x-3">
            ...
            <h2 class="text-2xl font-bold">Бронирование: рейс #{{ $voyage->voyage_number }}</h2>
        </div>
        <p class="mt-2 opacity-90">
            <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
            </svg>
            Время отправления: {{ $voyage->departure_time->format('d M Y, H:i') }}
        </p>
    </div>
    ...
    <div class="p-6 space-y-6">
        ...
        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div class="flex items-center space-x-4">
                <input 
                    ...
                    min="1" 
                    max="{{ min($voyage->available_seats, 8) }}"
                    oninput="this.value = this.value ? Math.max(1, Math.min({{ min($voyage->available_seats, 8) }}, parseInt(this.value) || 1)) : 1"
                ...
                >
                <span class="text-gray-500">Max: {{ min($voyage->available_seats, 8) }} мест</span>
            </div>
            @error('seats') 
        ...
        <div class="grid md:grid-cols-2 gap-4">
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                ...
                <p class="text-2xl font-bold text-blue-600">${{ number_format($voyage->price_per_seat, 2) }}</p>
            </div>
        ...
        <div class="text-center text-gray-500 text-sm mt-4">
            ...
            {{ $voyage->available_seats }} доступных мест осталось
        </div>
    </div>
</div>